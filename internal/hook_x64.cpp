...
// Implementation of the Hook class. The decoder covers the Vulkan loader stub
// prologue opcodes plus common function prologues (push/mov/sub/lea/endbr64).

namespace hook64 {

int Hook::modrm_len(const std::uint8_t* p, int modrm_off, int imm_bytes) {
...
int Hook::modrm_imm_bytes(std::uint8_t opcode) {
    return (opcode == 0x80 || opcode == 0x83 || opcode == 0xc6) ? 1 : 4;
}

int Hook::insn_len(const std::uint8_t* p) {
...
void Hook::emit_indirect_jmp(std::uint8_t* at, const void* target) {
    at[0] = 0xFF;
...
bool Hook::install(void* fn, void* detour) {
    ...
    std::uint8_t* p = base + patch_off;  // everything below works on the patch region
    has_endbr_ = has_endbr64;
    for (int i = 0; i < 14; ++i)
        orig_[patch_off + i] = p[i];
    int len = 0;
...
void Hook::uninstall() {
    if (!fn_) return;
    auto* base = static_cast<std::uint8_t*>(fn_);
    const int patch_off = has_endbr_ ? 4 : 0;
    const std::uintptr_t page = reinterpret_cast<std::uintptr_t>(base) & ~0xFFFULL;
    if (mprotect(reinterpret_cast<void*>(page), 0x1000,
                 PROT_READ | PROT_WRITE | PROT_EXEC) == 0) {
        std::memcpy(base + patch_off, orig_ + patch_off, 14);
        __builtin___clear_cache(reinterpret_cast<char*>(base + patch_off),
                                reinterpret_cast<char*>(base + patch_off) + 14);
        mprotect(reinterpret_cast<void*>(page), 0x1000, PROT_READ | PROT_EXEC);
    }
    if (trampoline_)
        munmap(trampoline_, static_cast<std::size_t>(stolen_len_) + 16);
    fn_ = nullptr;
    trampoline_ = nullptr;
    stolen_len_ = 0;
}
